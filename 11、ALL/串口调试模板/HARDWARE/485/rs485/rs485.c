...
unsigned char regGroup[5]={1,2,3,4,5};  //Modbus寄存器组，地址为0x00~0x04
...
void RS485_Init(u32 bound)
{
	...
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_13;	//CS-485
...
u8 _485Addr=0x01;	//设置_485Addr的默认地址
...
//									buf[len++]=regGroup[i++];		//低字节
									...
									switch (i)
									{
										case 0:		//电压	
															while(cnt--)
															{
																buf[len++]=AdcStructInitC.Voltage_S[pt++];				//寄存器高字节
																buf[len++]=AdcStructInitC.Voltage_S[pt++];		//低字节
															}
																break;
										case 1:		//电流
															while(cnt--)	
															{
																buf[len++]=AdcStructInitC.Current_S[pt++];				//寄存器高字节
																buf[len++]=AdcStructInitC.Current_S[pt++];		//低字节
															}
																break;
										case 2:		//功率
															while(cnt--)	
															{	
																buf[len++]=AdcStructInitC.Power_S[pt++];				//寄存器高字节
																buf[len++]=AdcStructInitC.Power_S[pt++];		//低字节
															}
																break;
										case 3:		//功率系数
															while(cnt--)	
															{	
																buf[len++]=AdcStructInitC.batteryPercentage_S[pt++];				//寄存器高字节
																buf[len++]=AdcStructInitC.batteryPercentage_S[pt++];		//低字节
															}
																break;
										case 4:		//占空比
															while(cnt--)	
															{	
																buf[len++]=Power_Max_Compare_Struct_Init.Present_Dk_S[pt++];				//寄存器高字节
																buf[len++]=Power_Max_Compare_Struct_Init.Present_Dk_S[pt++];		//低字节
															}
																break;
										case 5:  //通道号
